#let line-label(it) = {
  ...
  metadata((identifier: "lovelace line label", label: it))
}

#let normalize-line(line) = {
  if type(line) == content {
    (numbered: true, label: none, body: line)
...
#let no-number(body) = {
  if type(body) == content {
    (numbered: false, label: none, body: body)
...
#let with-line-label(label, body) = {
  if type(body) == content {
    (numbered: true, label: label, body: body)
  } else if type(body) == dictionary {
    body.label = label
    body
  }
...
#let line-number-figure(number, label, supplement, line-numbering) = {
  ...
  text(
    size: .8em, number-width: "tabular",
  )[#numbering(line-numbering, number)]

  if label != none {
    box[
      #figure(
        kind: "lovelace-line-number", supplement: supplement, numbering: line-numbering, none,
      )
      #label
    ]
  }
...
#let half-stroke(s) = {
  if s == none {
    ...
    (
      paint: s.paint, thickness: s.thickness / 2, cap: s.cap, join: s.join, dash: s.dash, miter-limit: s.miter-limit,
    )
...
#let identify-algorithm = context { let algos = query(figure.where(kind: "algorithm").before(here()))
if algos.len() > 0 { let algo = algos.last()

[#algo.supplement #algo.counter.display(algo.numbering)] } }

#let pseudocode(
  line-numbering: "1", line-number-supplement: "Line", stroke: 1pt + gray, indentation: 1em, hooks: 0pt, line-gap: .8em, booktabs-stroke: black + 2pt, booktabs: false, title: none, numbered-title: none, ..children,
) = {
  ...
  let collect-precursors(level, line-number, y, children) = {
    ...
    for child in children {
      if type(child) == dictionary {
        let content-precursor = (x: 2 * level, y: y, body: {
          set par(hanging-indent: .5em)
          set align(left)
          child.body
        }, numbered: child.numbered, kind: "content")
        precursors.push(content-precursor)
        if child.numbered {
          let number-precursor = (
            y: y, body: line-number-figure(line-number, child.label, line-number-supplement, line-numbering), kind: "number",
          )
        ...
        let nested-precursors = collect-precursors(level + 1, line-number, y, child)
        let nested-lines = nested-precursors.filter(p => p.kind == "content")
        let indent-precursor = (x: 2 * level + 1, y: y, rowspan: nested-lines.len(), kind: "indent")
  ...
  let cells = precursors.map(
    prec => {
      if prec.kind == "content" {
        grid.cell(
          x: prec.x + line-number-correction, y: prec.y + title-correction, colspan: max-x + 1 - prec.x, rowspan: 1, stroke: none, prec.body,
        )
      } else if prec.kind == "number" and line-numbering != none {
        grid.cell(
          x: 0, y: prec.y + title-correction, colspan: 1, rowspan: 1, stroke: none, prec.body,
        )
      } else if prec.kind == "indent" {
        grid.cell(
          x: prec.x + line-number-correction, y: prec.y + title-correction, colspan: 1, rowspan: prec.rowspan, stroke: (left: stroke, bottom: stroke, rest: none), h(hooks),
        )
      } else { () }
    },
  ).flatten()
  ...
  let decoration = (
    grid.header(
      grid.cell(
        x: 0, y: 0, colspan: max-x + 1 + line-number-correction, rowspan: 1, inset: if title != none { (y: .8em) } else { 0pt }, stroke: if title == none {
          (top: booktabs-stroke)
        } else {
          (top: booktabs-stroke, bottom: half-stroke(booktabs-stroke))
        }, align: left, title,
      ),
    ), grid.footer(
      repeat: false, grid.cell(
        y: max-y + 1, colspan: max-x + 1 + line-number-correction, rowspan: 1, stroke: (top: booktabs-stroke), none,
      ),
    ),
  )

  grid(
    columns: max-x + 1 + line-number-correction, column-gutter: indentation / 2, row-gutter: line-gap, ..cells, ..decoration,
  )
}

#let is-not-empty(it) = {
  return type(it) != content or not (
    it.fields() == (:) or (it.has("children") and it.children == ()) or (it.has("children") and it.children.all(c => not is-not-empty(c))) or (it.has("text") and it.text.match(regex("^\\s*$")) != none)
  )
}
  ...
  let transform-list(it, numbered) = {
    ...
    for child in it.children {
      ...
      if f in (enum.item, list.item) {
        ...
        child.func() == metadata and child.value.at("identifier", default: "") == "lovelace line label" and "label" in child.value
      ) {
